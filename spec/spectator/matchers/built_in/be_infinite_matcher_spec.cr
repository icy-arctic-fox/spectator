...
Spectator.describe BeInfiniteMatcher do
  ...
  describe "#does_not_match?" do
    ...
    it "returns false if the object is infinite" do
      matcher = BeInfiniteMatcher.new
      expect(matcher.does_not_match?(InfiniteObject.new)).to be_false
    end

    context "with a Float" do
      it "returns true if the value is finite" do
        matcher = BeInfiniteMatcher.new
        expect(matcher.does_not_match?(1.0)).to be_true
      end

      it "returns false if the value is positive infinity" do
        matcher = BeInfiniteMatcher.new
        expect(matcher.does_not_match?(Float64::INFINITY)).to be_false
      end
    ...
    context "with an Int" do
      it "returns true" do
        matcher = BeInfiniteMatcher.new
        expect(matcher.does_not_match?(1)).to be_true
      end
    end
  end
